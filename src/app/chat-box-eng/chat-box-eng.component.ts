...
import data from '../../assets/Models/ENG/data.json';
import dict from '../../assets/Models/ENG/dictionary.json';
import revDict from '../../assets/Models/ENG/revDictionary.json';
import * as tf from '@tensorflow/tfjs';
import { fetch as fetchPolyfill } from 'whatwg-fetch';
import { ModalService } from '../_modal';

@Component({
  selector: 'app-chat-box-eng',
  templateUrl: './chat-box-eng.component.html',
  styleUrls: ['./chat-box-eng.component.css']
})
export class ChatBoxEngComponent implements OnInit {
  chat: any;
  message: any;
  encoder: any;
  decoder: any;
  title = 'About eng chatbot';
  data: any;
  ...
  async loadModel() {
    ...
    this.encoder = await tf.loadLayersModel('assets/Models/ENG/Encoder_tf/model.json');
    console.log('Encoder loaded...');
    this.decoder = await tf.loadLayersModel('assets/Models/ENG/Decoder_tf/model.json');
  ...
  botReply(input_seq) {
    ...
    while (!stop_condition) {
      ...
      const sampledWord = revDict[sampledTokenIndex];
      if (sampledWord != 'bos' && sampledWord != 'eos') {
        decoded_sentence += sampledWord + ' ';
        word_count++;
      }
      if (sampledWord == 'eos' || word_count > data['dec_max_length']) {
        stop_condition = true;
      }
